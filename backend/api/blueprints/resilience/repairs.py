...
@resilience_repairs_blueprint.route('/get/', methods=["POST"])
@get_using_post_wrapper(table_name="repair", action="read", pass_user_id=False, pass_table_name=False)
def repair_get_single(request_dict):
    return database_resilience.disturbance_repair_get_single(request_dict["id"], "repair")

@resilience_repairs_blueprint.route('/get/', methods=["GET"])
...
@resilience_repairs_blueprint.route('/update_components/', methods=['POST'])
@post_wrapper(table_name="repair", action="update")
def repair_update_components(request_dict):
    return database_resilience.disturbance_repair_update_attributes(
        id=request_dict['id'],
